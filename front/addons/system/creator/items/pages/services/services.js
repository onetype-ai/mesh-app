	...
	pages.Item({
		...
		areas:
		{
			...
			main: function({ data })
			{
				...
				{
					$ot.page('/creator/services/' + value.id);
				};

				this.handleCreate = async () =>
				{
					const name = await $ot.confirm('Create service', 'Enter service name.', {
						input: true,
					...
					await $ot.command('services:create', {
						data: { name },
						redirect: 'creator'
					});
				};
				...
				return /* html */ `
					<div class="ot-container-l ot-py-l ot-flex-vertical">
						<e-global-heading
							title="Services"
							description="Long-running workloads you author — databases, AI runtimes, web apps. Publish to the marketplace when they are ready."
							size="m"
						>
							<e-form-button slot="right" text="Create" color="brand" icon="add" :_click="handleCreate"></e-form-button>
						</e-global-heading>

						<e-creator-tabs active="services"></e-creator-tabs>
