onetype.AddonReady('pages', (pages) =>
{
	pages.Item({
		...
		data: async function(parameters)
		{
			...
			return { service: item.data, scripts: list.map((script) => script.data) };
		},
		areas:
		{
			sidebar: function()
			...
			main: function({ data })
			{
				...
				this.handleImport = async () =>
				{
					const confirmed = await $ot.confirm(
						'Import ' + this.service.name,
						'A private copy will be added to your library. You can deploy it to your servers and edit it anytime.',
					...
					await $ot.command('marketplace:service:import', { service_id: this.service.id });
					...
					<div class="ot-flex-vertical ot-gap-l">
						...
						<div class="ot-container-m ot-pb-l marketplace-body">
							<div class="marketplace-main">
								<e-global-markdown ot-if="!script && service.overview" :content="service.overview"></e-global-markdown>
