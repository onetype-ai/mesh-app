...
onetype.PipelineOn('commit', 'servers:scripts:detach', async function(pipeline, result, properties)
{
	...
	const action  = 'servers.scripts.detach';
...
onetype.PipelineOn('rollback', 'servers:scripts:detach', async function(pipeline, result, properties, error)
{
	if(result.id.parent !== null)
	{
		return;
