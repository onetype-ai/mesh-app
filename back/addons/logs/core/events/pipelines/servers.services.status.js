...
onetype.PipelineOn('commit', 'servers:services:status', async function(pipeline, result, properties)
{
	...
	const action  = 'servers.services.status';
...
onetype.PipelineOn('rollback', 'servers:services:status', async function(pipeline, result, properties, error)
{
	if(result.id.parent !== null)
	{
		return;
