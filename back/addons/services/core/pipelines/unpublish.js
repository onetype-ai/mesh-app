...
onetype.Pipeline('services:unpublish', {
	description: 'Move a service back to Draft. No cascade — services are leaf nodes and nothing references them.',
	in: {
		id: ['string', null, true]
	}
})
...
.Join('load', 10, {
	...
	{
		const item = await services.Find().filter('id', id).filter('deleted_at', null, 'NULL').one();
