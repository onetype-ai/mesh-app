...
.Join('stop', 40, {
	description: 'Run the stop script on the server.',
	requires: ['server', 'service'],
	callback: async function({ server, service }, resolve)
	{
		if(!service.Get('script_stop_id'))
		{
			return resolve(null, 'Service does not declare a stop script.', 400);
		}

		const stop = await scripts.Find()
			.filter('id', service.Get('script_stop_id'))
			.filter('deleted_at', null, 'NULL')
			.one();

		if(!stop)
		{
			return resolve(null, 'Stop script missing.', 500);
		}

		const result = await this.Pipeline('agents:bash', {
			agent_id: server.Get('id'),
			bash: stop.Get('bash'), timeout: 1800000
		});

		if(result.code !== 0)
		{
			return resolve(null, 'Stop script exited with code ' + result.code + '.', 500);
		}
	}
});
