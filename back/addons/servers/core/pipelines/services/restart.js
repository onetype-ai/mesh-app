...
onetype.Pipeline('servers:services:restart', {
	description: 'Restart a deployed service — requires the service to be deployed first.',
	in: {
...
.Join('restart', 40, {
	description: 'Run the restart script on the server.',
	requires: ['server', 'service'],
	callback: async function({ server, service }, resolve)
	{
		if(!service.Get('script_restart_id'))
		{
			return resolve(null, 'Service does not declare a restart script.', 400);
		}

		const restart = await scripts.Find()
			.filter('id', service.Get('script_restart_id'))
			.filter('deleted_at', null, 'NULL')
			.one();

		if(!restart)
		{
			return resolve(null, 'Restart script missing.', 500);
		}

		const result = await this.Pipeline('agents:bash', {
			agent_id: server.Get('id'),
			bash: restart.Get('bash'), timeout: 1800000
		});

		if(result.code !== 0)
		{
			return resolve(null, 'Restart script exited with code ' + result.code + '.', 500);
		}
	}
});
