...
onetype.Pipeline('servers:services:start', {
	description: 'Start a deployed service — requires the service to be deployed first.',
	in: {
...
.Join('start', 40, {
	description: 'Run the start script on the server.',
	requires: ['server', 'service'],
	callback: async function({ server, service }, resolve)
	{
		if(!service.Get('script_start_id'))
		{
			return resolve(null, 'Service does not declare a start script.', 400);
		}

		const start = await scripts.Find()
			.filter('id', service.Get('script_start_id'))
			.filter('deleted_at', null, 'NULL')
			.one();

		if(!start)
		{
			return resolve(null, 'Start script missing.', 500);
		}

		const result = await this.Pipeline('agents:bash', {
			agent_id: server.Get('id'),
			bash: start.Get('bash'), timeout: 1800000
		});

		if(result.code !== 0)
		{
			return resolve(null, 'Start script exited with code ' + result.code + '.', 500);
		}
	}
});
